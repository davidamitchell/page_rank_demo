...
Matrix.prototype.eigenvector = function() {
  ...
  for (var i = 0; i < max; i++) {
    ...
    if ( Math.abs( difference ) < tolerance ) {
      console.log('[by matrix] breaking at convergence!!!!  ', i, ' iterations');
      break;
    }
  ...
  PageRank.prototype.rank = function( graph, damping_value ) {
    ...
    for ( var i = 0; i < max; ++i ) {
      ...
      for ( var j = 0; j < size; ++j ) {
        ...
        var rank = (1-damping_value) + ( damping_value * linking_ranks )
      ...
      if ( difference < tolerance ){
        console.log('[by formula] breaking at convergence!!!!  ', i, ' iterations');
        break;
      }
    }
