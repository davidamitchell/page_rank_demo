...
Vector.prototype.total_difference = function( vector ) {
  var diff   = this.subtract( vector )
  var length = diff.elements.length;
  var total  = 0;

  for (var i = 0; i < length; i++) {
    total += diff.elements[ i ];
  }
  return total;
}
...
Matrix.prototype.eigenvector = function() {
  ...
  for (var i = 0; i < max; i++) {
    ...
    var difference = 100 * ( c_new - c_old )/c_new;
